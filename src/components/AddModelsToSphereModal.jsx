...
export default function AddModelsToSphereModal({ sphereId, onClose, onAdd, existingModelIds = [] }) {
  ...
  const sortedSpheres = [...spheres].sort((a, b) => {
    const aCount = availableModels.filter(model => model.spheres?.some(s => s.id === a.id)).length
    const bCount = availableModels.filter(model => model.spheres?.some(s => s.id === b.id)).length
