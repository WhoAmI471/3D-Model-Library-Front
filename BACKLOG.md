...
### 7. Добавить глобальные оповещения для всех действий с загрузкой

**Статус:** ✅ Завершено

**Описание:**
Добавить систему глобальных оповещений (уведомлений), которые будут показываться при всех операциях изменения, добавления и удаления данных. Оповещения должны отображаться в стиле дизайна сайта, с плавной анимацией выезжания сверху, располагаться чуть левее от имени пользователя в хедере.

**Задачи:**
- [x] Добавить задачу в BACKLOG
- [x] Создать компонент `Notification` с анимацией выезжания сверху
- [x] Создать контекст `NotificationProvider` для глобального управления уведомлениями
- [x] Интегрировать компонент уведомлений в `MainHeader` (слева от имени пользователя, сверху)
- [x] Поддержка типов уведомлений: success, error, info, loading
- [x] Создать хук `useNotification` для удобного использования
- [x] Исправить позиционирование уведомлений (выше, правее, в рамках header)
- [x] Убрать оповещение загрузки модели (использовать только индикатор прогресса)
- [x] Добавить оповещения при создании модели (`ModelUploadForm.jsx` - успех/ошибка)
- [x] Добавить оповещения при обновлении модели (`ModelEditForm.jsx` - успех/ошибка уже есть, проверить)
- [x] Добавить оповещения при создании/обновлении проекта (`projects/page.js` - handleProjectSubmit)
- [x] Добавить оповещения при удалении проекта (`projects/page.js` - handleDelete)
- [x] Добавить оповещения при запросе удаления проекта (`projects/[id]/page.js` - handleDeleteRequest)
- [x] Добавить оповещения при подтверждении удаления проекта (`projects/[id]/page.js` - handleDeleteConfirm)
- [x] Добавить оповещения при создании/обновлении сотрудника (`employees/page.js` - handleEmployeeSubmit)
- [x] Добавить оповещения при удалении сотрудника (`employees/page.js` - handleDelete)
- [x] Добавить оповещения при создании сферы (`spheres/page.js` - handleSphereSubmit)
- [x] Добавить оповещения при удалении сферы (`spheres/page.js` - handleDelete)
- [x] Добавить оповещения при запросе удаления модели (`dashboard/page.js` - handleDeleteRequest)
- [x] Добавить оповещения при подтверждении удаления модели (`dashboard/page.js` - handleDeleteConfirm)
- [x] Добавить оповещения при запросе удаления модели (`models/[id]/page.js` - handleDeleteRequest)
- [x] Добавить оповещения при решении администратора (`AdminDeletionPanel.jsx` - handleDecision)
- [x] Заменить все `alert()` на уведомления
- [x] Заменить все `confirm()` на модальные окна с уведомлениями

**Затронутые файлы:**
- `src/components/Notification.jsx` (новый компонент)
- `src/components/NotificationContainer.jsx` (новый компонент)
- `src/contexts/NotificationContext.jsx` (новый контекст)
- `src/hooks/useNotification.js` (новый хук)
- `src/components/ConfirmModal.jsx` (новый компонент для подтверждения)
- `src/hooks/useConfirm.js` (новый хук для подтверждения)
- `src/components/MainHeader.jsx` (интеграция компонента уведомлений)
- `src/app/layout.js` (добавление провайдера)
- `src/components/ModelEditForm.jsx` (добавление уведомлений)
- `src/components/ModelUploadForm.jsx` (добавление уведомлений)
- `src/components/ModelCard.jsx` (замена confirm на модальные окна)
- `src/components/ProjectForm.jsx` (добавление уведомлений)
- `src/components/EmployeeForm.jsx` (добавление уведомлений)
- `src/components/AdminDeletionPanel.jsx` (добавление уведомлений)
- `src/components/modelForm/ScreenshotsSection.jsx` (замена alert на уведомления)
- `src/components/modelForm/ScreenshotsUploadSection.jsx` (замена alert на уведомления)
- `src/app/dashboard/projects/page.js` (добавление уведомлений и модальных окон)
- `src/app/dashboard/employees/page.js` (добавление уведомлений и модальных окон)
- `src/app/dashboard/spheres/page.js` (добавление уведомлений и модальных окон)
- `src/app/dashboard/page.js` (добавление уведомлений и модальных окон)
- `src/app/dashboard/projects/[id]/page.js` (добавление уведомлений и модальных окон)

**Ожидаемый результат:**
- Единообразная система оповещений по всему приложению
- Улучшенный UX с визуальной обратной связью для всех действий
- Плавные анимации выезжания уведомлений сверху
- Интеграция в существующий дизайн сайта

---

## Прогресс

### Общий прогресс: 6/7 задач завершено (86%) 🎉
